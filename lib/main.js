...
class HTMLLanguageClient extends AutoLanguageClient {
  ...
  // Transform a completion suggestion in a custom fashion.
  onDidConvertAutocomplete(completionItem, suggestion){
    // `atom.ui.markdown` makes a hash of inline SVG images in Markdown, so
    // we'll pre-parse them with `marked`.
    if (completionItem.documentation?.kind === 'markdown') {
      suggestion.descriptionMarkdown = marked.parse(completionItem.documentation.value);
    }
  }
